...
class ONEILL_PT_alignment_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("oneill.align_cylinders", text="Align Selected")
...
class ONEILL_PT_terrain_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Actions
        col = layout.column(align=True)
        col.operator("oneill.create_heightmap_canvas", text="Create Heightmap", icon='IMAGE_DATA')
        col.operator("oneill.rewrap_cylinder", text="Rewrap to Cylinder", icon='MESH_CYLINDER')
